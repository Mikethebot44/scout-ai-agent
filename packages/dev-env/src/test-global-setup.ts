...
export async function setupTestContainers(): Promise<SetupResult> {
  ...
  try {
    ...
    execSync(
      'docker exec scout_postgres_test psql -U postgres -d postgres -c "DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public;"',
      {
        stdio: "inherit",
      },
    );
    // Clear Redis data
    execSync("docker exec scout_redis_test redis-cli FLUSHALL", {
      stdio: "inherit",
    });
  } catch (error) {
