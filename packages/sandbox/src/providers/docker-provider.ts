...
const REPO_DIR = "repo";
const BASE_IMAGE = "ghcr.io/scout-labs/containers-test";
const SLEEP_MS = 60 * 60 * 1000; // 1 hour

const CONTAINER_PREFIX = "scout-sandbox";
...
export class DockerProvider implements ISandboxProvider {
  ...
  /**
   * Cleanup utility function to remove all Scout containers
   * Useful for complete system cleanup
   */
  static async cleanupAllContainers(): Promise<void> {
    try {
      // Get all containers with any scout prefix
      ...
      console.warn("Failed to cleanup Scout containers:", error);
    }
  }
}
