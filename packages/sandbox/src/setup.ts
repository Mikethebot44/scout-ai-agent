...
import { AIAgent, AIAgentCredentials } from "@scout/agent/types";
...
export async function setupSandboxOneTime(
  ...
  // Only run scout-setup.sh if not explicitly skipped
  ...
  const result = await Promise.race([
    (async () => {
      await session.runCommand(command, {
        timeoutMs: scoutSetupScriptTimeoutMs,
    ...
    new Promise<"timeout">((resolve) =>
      setTimeout(() => resolve("timeout"), scoutSetupScriptTimeoutMs),
    ),
  ]);
  if (result === "timeout") {
    throw new Error(
      `Command timed out after ${scoutSetupScriptTimeoutMs}ms`,
    );
  }
...
export async function runSetupScript({
  ...
  const customScriptPath =
    options.setupScriptPath || "/tmp/scout-setup-custom.sh";
  ...
  try {
    // If a custom setup script is provided, use it instead of checking for scout-setup.sh
    if (options.setupScript) {
      ...
      // Use the repository's scout-setup.sh if it exists
      await executeSetupScriptCommand({
        session,
        command:
          "bash -c 'if [ -f scout-setup.sh ]; then chmod +x scout-setup.sh && bash -x ./scout-setup.sh; fi'",
        environmentVariables: options.environmentVariables,
        agentCredentials: options.agentCredentials,
        githubAccessToken: options.githubAccessToken,
        onUpdate: onUpdateWrapped,
