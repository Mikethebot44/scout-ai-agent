...
import { encryptToken } from "@scout/utils/encryption";
...
import { getUnusedPromotionCodeForUser } from "@scout/shared/model/subscription";
import {
  STRIPE_PLAN_CONFIGS,
    ...
    magicLink({
      ...
      sendMagicLink: async ({ email, url: rawUrl }) => {
        const url =
          process.env.NEXT_PUBLIC_VERCEL_ENV === "preview"
            ? rawUrl.replace(
                "https://www.scoutlabs.com",
                `https://${process.env.NEXT_PUBLIC_VERCEL_BRANCH_URL}`,
              )
            : rawUrl;
        const resend = new Resend(env.RESEND_API_KEY ?? "DUMMY_KEY");
        const result = await resend.emails.send({
          from: "Terry <onboarding@mail.scoutlabs.com>",
          to: email,
          subject: "Sign in to Scout",
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2>Sign in to Scout</h2>
              <p>Hello,</p>
              <p>Click the link below to sign in to your Scout account:</p>
              <a href="${url}" style="display: inline-block; padding: 12px 24px; background-color: #000; color: #fff; text-decoration: none; border-radius: 6px; margin: 16px 0;">Sign in to Scout</a>
