...
} from "@scout/shared/github-api/helpers";
import { getInstallationToken } from "@scout/shared/github-app";
import { getPostHogServer } from "./posthog-server";
import { publishBroadcastUserMessage } from "@scout/shared/broadcast-server";
import { BroadcastMessageThreadData } from "@scout/types/broadcast";
import { updateThread } from "@scout/shared/model/threads";
import {
    ...
    if (error.status === 404) {
      try {
        ...
        if (branches.length === 0) {
          ...
          const readmeContent = `# ${repoName}\n\nThis repository was initialized by Scout.`;
