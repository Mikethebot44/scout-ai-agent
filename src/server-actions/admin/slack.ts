...
import { getSlackAccountAndInstallationForWorkspace } from "@scout/shared/model/slack";
import { decryptValue } from "@scout/utils/encryption";
...
> {
  ...
  if (!slackInstallation) {
    return {
      success: false,
      error: `No Slack installation found for workspace: ${workspaceDomain}. Make sure the workspace is connected to Scout.`,
    };
  }
  if (!slackAccount) {
    return {
      success: false,
      error: `No Slack account found for workspace: ${workspaceDomain}. Make sure the workspace is connected to Scout.`,
    };
  }
