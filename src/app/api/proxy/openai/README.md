...
- The server injects `Authorization: Bearer <OPENAI_API_KEY>` from `@scout/env/apps-www`.
- Clients must include `X-Daemon-Token: <Scout API key>` in each request; the proxy verifies the key using `auth.api.verifyApiKey` (same mechanism as the daemon event handler).
- Access additionally requires that the requesting user have a positive Scout credit balance; requests from users without remaining credits receive a `402 Payment Required` response.
...
Point Codex at the proxy by defining a Scout model provider in your `config.toml` and setting the daemon token when invoking Codex:
...
[model_providers.terry]
name = "terry"
base_url = "http://scoutlabs.com/api/proxy/openai/v1"
env_http_headers = { "X-Daemon-Token" = "DAEMON_TOKEN" }
