...
- Clients must include `X-Daemon-Token: <Scout API key>` in each request; the proxy verifies the key using `auth.api.verifyApiKey`
  ...
  "provider": {
    "terry": {
      ...
      "options": {
        ...
        "headers": {
          "X-Daemon-Token": "{env:SCOUT_TOKEN}"
        }
      },
      "models": {
        "grok-code": {
...
```sh
SCOUT_TOKEN=<token> opencode run --model terry/grok-code --format json "Hi, how are you?"
```
