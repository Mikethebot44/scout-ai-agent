...
vi.mock("@scout/shared/github-app", () => ({
...
vi.mock("@scout/sandbox", async (importOriginal) => {
  const actual = (await importOriginal()) as any;
  return {
    ...actual,
...
vi.mock("@scout/sandbox/commands", async (importOriginal) => {
  const actual = (await importOriginal()) as any;
  const mockGitDiff = `
